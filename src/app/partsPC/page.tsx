...
export default function PartsPC() {
  const [partsPCs, setPartsPCs] = useState<Product[]>([]);
  const [filteredPartsPCs, setFilteredPartsPCs] = useState<Product[]>([]);
  const [displayedPartsPCs, setDisplayedPartsPCs] = useState<Product[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const { data: partsPCsData, error: partsPCsError } = await supabase
          .from("products")
          .select(
            `
            *,
        ...
        if (partsPCsError) throw new Error(partsPCsError.message);
        ...
        const formattedPartsPCs = (partsPCsData || []).map((item: any) => ({
          ...item,
          reviews: item.reviews || [],
          priceRange: "",
          isNew: false,
        }));
        setPartsPCs(formattedPartsPCs);
        setFilteredPartsPCs(formattedPartsPCs);

        const prices = formattedPartsPCs.map((pc: Product) => pc.price);
  ...
  useEffect(() => {
    ...
    setDisplayedPartsPCs(filteredPartsPCs.slice(startIndex, endIndex));
  }, [filteredPartsPCs, currentPage, itemsPerPage]);
  ...
  const totalPages = Math.ceil(filteredPartsPCs.length / itemsPerPage);
  ...
  return (
    <main className={styles.partsPCPage}>
      <section className={styles.topSection}>
        ...
        <div className={styles.header}>
          <h1 className={styles.title}>PC Parts ({filteredPartsPCs.length})</h1>
          <div className={styles.navControls}>
            <span className={styles.itemsCount}>
              Items {(currentPage - 1) * itemsPerPage + 1}-
              {Math.min(currentPage * itemsPerPage, filteredPartsPCs.length)} of{" "}
              {filteredPartsPCs.length}
            </span>
      ...
      <section className={styles.mainSection}>
        ...
        >
          {displayedPartsPCs.length > 0 ? (
            displayedPartsPCs.map((partPC) => (
              <div
                key={partPC.id}
                className={
