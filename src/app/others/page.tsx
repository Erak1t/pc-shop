...
export default async function Others() {
  ...
  if (othersError) {
    console.error("Error fetching accessories:", othersError);
    ...
    <main>
      <section className={styles.topSection}>
        ...
        <div className={styles.header}>
          <h1 className={styles.title}>Accessories ({others?.length || 0})</h1>
          <div className={styles.navControls}>
            <span className={styles.itemsCount}>
              Items 1-{others?.length || 0} of {others?.length || 0}
            </span>
        ...
        <div className={styles.filters}>
          ...
          <div className={styles.filterButtons}>
            <button className={styles.filterButton} disabled>
              Accessories ({others?.length || 0}){" "}
              <span className={styles.removeFilter}>×</span>
            </button>
            <button className={styles.filterButton} disabled>
              In Stock (
              {others?.filter((item: Product) => item.stock === "in stock")
                .length || 0}
              ) <span className={styles.removeFilter}>×</span>
            </button>
          </div>
