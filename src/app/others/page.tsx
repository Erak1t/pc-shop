...
export default function Others() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const formattedAccessories = (accessoriesData || []).map(
          (item: any) => ({
            ...item,
        ...
        setFilteredAccessories(formattedAccessories);

        const prices = formattedAccessories.map((item: Product) => item.price);
  ...
  useEffect(() => {
    ...
    setDisplayedAccessories(filteredAccessories.slice(startIndex, endIndex));
  ...
  const getFilteredCount = (
    ...
    if (filterType === "color" && value) {
      return filtered.filter((item) => item.color === value).length;
    }
    if (filterType === "brand" && value) {
      return filtered.filter((item) => item.name.split(" ")[0] === value)
        .length;
    }
    if (filterType === "stock") {
      return filtered.filter((item) => item.stock === "in stock").length;
    }
    return 0;
  };
  ...
  return (
    <main className={styles.othersPage}>
      <section className={styles.topSection}>
        <div className={styles.breadcrumb}>
          ...
          <Link href="/others" className={styles.breadcrumbLink}>
            Accessories
          </Link>
        </div>
        <div className={styles.header}>
          ...
          <div className={styles.navControls}>
            <span className={styles.itemsCount}>
              ...
              {Math.min(currentPage * itemsPerPage, filteredAccessories.length)}{" "}
              of {filteredAccessories.length}
            </span>
