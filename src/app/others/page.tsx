...
// Фільтруємо лише продукти з категорії "others"
const others = productsData.filter(
  (product: Product) => product.category === "others"
);
    ...
    <main>
      <section className={styles.topSection}>
        ...
        <div className={styles.header}>
          <h1 className={styles.title}>Accessories ({others.length})</h1>
          <div className={styles.navControls}>
            <span className={styles.itemsCount}>
              Items 1-{others.length} of {others.length}
            </span>
        ...
        <div className={styles.filters}>
          ...
          <div className={styles.filterButtons}>
            <button className={styles.filterButton}>
              Accessories ({others.length}){" "}
              <span className={styles.removeFilter}>×</span>
            </button>
            <button className={styles.filterButton}>
              In Stock (
              {others.filter((item) => item.stock === "in stock").length}){" "}
              <span className={styles.removeFilter}>×</span>
            </button>
          </div>
